...
from dotenv import load_dotenv  # Adicionado temporariamente para teste

# Carrega variáveis de ambiente temporariamente
load_dotenv()
...
# Contexto para manter histórico de perguntas
contexto = []

# Definição de intenções e respostas
intencoes = {
...
class BotWorker:
    def __init__(self):
        """Inicializa o worker com configurações do .env."""
        ...
        if not all([self.wolfram_app_id, self.google_cx, self.google_api_key]):
            logger.warning("Algumas chaves de API não estão configuradas no .env")
        logger.info("BotWorker inicializado com sucesso.")

    def process_query(self, query: str, usuario_id: int = None) -> dict:
        """Processa a pergunta e retorna a resposta formatada com metadados."""
    ...
    def _traduzir(self, texto: str, origem: str = "auto", destino: str = "pt") -> str:
        """Traduz o texto usando Google Translator."""
        try:
    ...
    def _responder(self, intencao: str, mensagem: str) -> tuple:
        """Gera a resposta com base na intenção e retorna a fonte."""
        global contexto
        mensagem_limpa = self._normalizar_texto(mensagem)
    ...
    def _get_bot_response(self, pergunta: str) -> tuple:
        """Obtém a resposta do bot e a fonte."""
        try:
